...
- The consumer baseline is committed DoctorApp `origin/main` `DoctorProfileApproved`: it requires the full identity/display payload and the top-level `doctorFee`, `doctorFeeCurrency`, `languages`, `durationMinutes`, `channels`, `approvedAt`, and `occurredAt` fields. See `specs/depends-on/doctor-profile-approved.asyncapi.yaml`.
